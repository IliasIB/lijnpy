...
from lijnpy.kern_open_data_services_api.v1.models import Color
...
def get_colors() -> list[Color]:
    ...
    Returns:
        Colors: A list of all colors
    """
    result = _rest_adapter.get("/kleuren")
    try:
        assert result.data is not None
        colors = [Color(**color) for color in result.data["kleuren"]]
...
def get_color(color_code: str) -> Color:
